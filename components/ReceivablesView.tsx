...
const ReceivablesView: React.FC<ReceivablesViewProps> = ({ transactions, addPayment }) => {
    ...
    const creditTransactions = useMemo(() => {
        return transactions.filter(t => t.isCredit && !t.isPaid && t.type === TransactionType.Sale);
    }, [transactions]);
        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-24">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Cuentas por Cobrar</h2>
                </div>
                ...
                <div className="space-y-4">
                    {creditTransactions.length > 0 ? (
                            ...
                            return (
                                <div key={t.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow bg-gray-50 dark:bg-gray-700/30">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <h3 className="font-semibold text-gray-800 dark:text-gray-200 text-lg">{t.clientName || "Cliente Desconocido"}</h3>
                        ...
                        <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                            <p>No tienes cuentas por cobrar pendientes.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {isModalOpen && selectedTransaction && (
                <div className="fixed inset-0 z-50 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4" onClick={handleClosePaymentModal}>
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6 animate-fade-in-scale-up" onClick={e => e.stopPropagation()}>
                        <h3 className="text-xl font-bold mb-4 text-gray-800 dark:text-gray-100">Registrar Abono</h3>
                        <div className="mb-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
                            <p className="text-sm text-gray-600 dark:text-gray-300">Cliente: <span className="font-semibold">{selectedTransaction.clientName}</span></p>
